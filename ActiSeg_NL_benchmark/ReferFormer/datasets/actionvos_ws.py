...
class ActionVOSDatasetWS(Dataset):
    def __init__(self, actionvos_folder: Path, imagesets_path: Path,jpegimages_path: Path,annotations_path: Path, weights_path: Path,
                 ann_file: Path, transforms, return_masks: bool, 
    ...
    def _load_or_create_index_map(self):
        ...
        if os.path.exists(self.index_map_path):
            ...
            with open(self.imagesets_path / f"{self.image_set}_objects_category.json", "r") as f:
                subset_metas_by_video = json.load(f)['videos']
            # with open(os.path.join(str(self.actionvos_folder), 'ImageSets', self.ann_file), 'r') as f:
            with open(self.imagesets_path / self.ann_file, "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        for j in range(self.num_frames):
            ...
            img_path  = self.jpegimages_path / self.image_set / video / f"{frame_name}.jpg"
            mask_path = self.annotations_path / self.image_set / video / f"{frame_name}.png"
            img = Image.open(img_path).convert('RGB')
            mask = Image.open(mask_path).convert('P')
            if self.use_weights:
                # weight_path = os.path.join(str(self.actionvos_folder), 'Weights_Sparse', self.image_set, video, frame_name + '.png')
                weight_path = self.weights_path / self.image_set / video / f"{frame_name}.png"
        ...
        # return imgs, target
        # Modified: handle dual augmentation for training set
        if self.image_set == 'train':
            imgs_w, imgs_s, target = self._transforms(imgs, target)
            imgs_w = torch.stack(imgs_w, dim=0)  # Weak augmentation image stack
            imgs_s = torch.stack(imgs_s, dim=0)  # Strong augmentation image stack
            return imgs_w, imgs_s, target
        else:
            imgs, target = self._transforms(imgs, target)
            imgs = torch.stack(imgs, dim=0)
            return imgs, target
class CLDataTransform(object):
    ...
    def __call__(self, imgs, target):
        ...
        # imgs_s = []
        # for img in imgs:
            # imgs_w.append(self.transform_weak(img, target)[0])  # Weak augmentation image
            # imgs_s.append(self.transform_strong(img, target)[0])  # Strong augmentation image
        
        imgs_w, _ = self.transform_weak(imgs, target)
        imgs_s, target = self.transform_strong(imgs, target)
...
def make_coco_transforms(image_set, max_size=640):
    ...
    if image_set == 'train':
        ...
        strong_transform = T.Compose([
            T.PhotometricDistort(),  # Photometric distortion
            T.RandomResize([480], max_size=max_size),  # Fixed scaling
            normalize,
        ])
        return CLDataTransform(weak_transform, strong_transform)    
...
def build(image_set, args):
    ...
    print("args.sample_map_path",args.sample_map_path)
    # Path parsing strategy consistent with the above
    ...
    dataset = ActionVOSDatasetWS(args.actionvos_path, imagesets_path=imagesets_path,jpegimages_path=jpegimages_path,
                                annotations_path=annotations_path,weights_path=weights_path, ann_file=ann_file,

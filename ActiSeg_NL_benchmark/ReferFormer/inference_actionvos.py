...
# build transform
transform = T.Compose([
    T.Resize(360),#Resize shortest side to 360, maintain aspect ratio
    T.ToTensor(),
        ...
        for i in range(num_expressions):
            ...
            for clip_id in range(0, video_len, 36):
                ...
                for t in clip_frames_ids:
                    ...
                    origin_w, origin_h = img.size#Used for interpolating output results to obtain predicted annotations
                ...
                with torch.no_grad():
                    outputs = model([imgs], [exp], [target])#####Check if multi-frame input is required

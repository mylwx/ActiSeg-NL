...
from .segmentation import (dice_loss, sigmoid_focal_loss, sigmoid_focal_loss_weighted,sce_loss,gce_loss,active_passive_loss)
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        return losses
    def loss_weighted_sce_masks(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
           targets dicts must contain the key "weights" containing a tensor of dim [nb_target_boxes, h, w]
           weights are for focal loss, should be the same shape as target['masks']
        ...
        losses = {
            "loss_dice": dice_loss(src_masks, target_masks, num_boxes),
            "loss_sce": sce_loss(src_masks, target_masks, num_boxes, weights),
        }
        return losses

    def loss_weighted_gce_masks(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
           targets dicts must contain the key "weights" containing a tensor of dim [nb_target_boxes, h, w]
           weights are for focal loss, should be the same shape as target['masks']
        ...
        losses = {
            "loss_dice": dice_loss(src_masks, target_masks, num_boxes),
            "loss_gce": gce_loss(src_masks, target_masks, num_boxes, weights),
        }
        return losses

    def loss_sce_masks(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
           targets dicts must contain the key "weights" containing a tensor of dim [nb_target_boxes, h, w]
           weights are for focal loss, should be the same shape as target['masks']
        ...
        losses = {
            "loss_dice": dice_loss(src_masks, target_masks, num_boxes),
            "loss_sce": sce_loss(src_masks, target_masks, num_boxes, weights=None),
        }
        return losses

    def loss_gce_masks(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
           targets dicts must contain the key "weights" containing a tensor of dim [nb_target_boxes, h, w]
           weights are for focal loss, should be the same shape as target['masks']
        ...
        losses = {
            "loss_dice": dice_loss(src_masks, target_masks, num_boxes),
            "loss_gce": gce_loss(src_masks, target_masks, num_boxes, weights=None),
        }
        return losses
    
    def loss_active_passive_masks(self, outputs, targets, indices, num_boxes):
        assert "pred_masks" in outputs

        src_idx = self._get_src_permutation_idx(indices)
        src_masks = outputs["pred_masks"].transpose(1, 2)
        ...
        # Use active_passive_loss
        losses = {
            "loss_dice": dice_loss(src_masks, target_masks, num_boxes),
            "loss_active_passive": active_passive_loss(src_masks, target_masks, num_boxes=num_boxes, weights=weights, 
                                                    active='gce', passive='mae'),
        }
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {#loss_weighted_sce_masks##loss_weighted_gce_masks
            ...
            'active_passive_masks': self.loss_active_passive_masks,
        }

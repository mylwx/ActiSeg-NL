...
####### ELR loss
# from .criterion_elr import SetCriterionELR

# from .criterion_elr_orgsize import SetCriterionELR as SetCriterionELROrgSize
from .criterion_elr_orgsize_mask import SetCriterionELR as SetCriterionELROrgSizeMask
###### NPN loss
# from .criterion_npn_orgsize import SetCriterionNPN as SetCriterionNPNOrgSize
from .criterion_npn_orgsize_mask import SetCriterionNPN as SetCriterionNPNOrgSizeMask
...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
                    ...
                    positive_cls=False,has_class_category_embed = False,num_category_classes = 1):#has_class_category_embed = False,num_category_classes = 1
        ...
        self.has_class_category_embed = has_class_category_embed
        ### extra category head
        if self.has_class_category_embed:
            self.num_category_classes = num_category_classes
            # self.category_embed = nn.Linear(hidden_dim, num_category_classes)
        ...
        if self.has_class_category_embed:
        #     self.catogory_embed.bias.data = torch.ones(num_category_classes) * bias_value
            category_embed = nn.Linear(hidden_dim, num_category_classes)  # 先定义单个线性层
            self.category_embed = _get_clones(category_embed, num_pred)  # 克隆为 ModuleList
            # 对每个层的偏置进行初始化
            prior_prob = 0.01
            bias_value = -math.log((1 - prior_prob) / prior_prob)
            for layer in self.category_embed:
                layer.bias.data = torch.ones(num_category_classes) * bias_value
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if self.has_class_category_embed:
            outputs_categories = []
            for lvl in range(hs.shape[0]):
                outputs_category = self.category_embed[lvl](hs[lvl])
                outputs_categories.append(outputs_category)
            outputs_categories = torch.stack(outputs_categories)
            outputs_categories = rearrange(outputs_categories, 'l (b t) q k -> l b t q k', b=b, t=t)
            out['pred_category_logits'] = outputs_categories[-1]
        ...
        if self.aux_loss:
            ...
            if self.has_class_category_embed:
                if self.positive_cls:
                    out['aux_outputs'] = self._set_aux_loss_pos_category(outputs_class, outputs_coord, outputs_seg_masks, outputs_positives, outputs_categories)
                else:
                    out['aux_outputs'] = self._set_aux_loss_catogory(outputs_class, outputs_coord, outputs_seg_masks, outputs_categories)
    ...
    @torch.jit.unused
    def _set_aux_loss_catogory(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_categories):
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_category_logits": d} 
                for a, b, c, d in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_categories[:-1])]

    @torch.jit.unused
    def _set_aux_loss_pos_category(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_positives, outputs_categories):
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_positives": d, "pred_category_logits": e} 
                for a, b, c, d, e in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_positives[:-1], outputs_categories[:-1])]
...
def build(args):
    if args.binary:
        num_classes = 1
        if args.has_class_category_embed:
            num_category_classes = args.num_category_classes
        else:
            num_category_classes = 1
    else:
        if args.dataset_file == 'ytvos':
            num_classes = 65
            num_category_classes = 65  # YTVOS 有 65 个类别
        elif args.dataset_file == 'davis':
            num_classes = 78
            num_category_classes = 78  # Davis 有 78 个类别
        elif args.dataset_file == 'a2d' or args.dataset_file == 'jhmdb':
            num_classes = 1
            num_category_classes = 1  # A2D 和 JHMDB 通常为单一类别
        else:
            num_classes = 91
            num_category_classes = 91  # COCO 有 91 个类别
    # if args.binary:
    #     num_classes = 1
    # else:
    #     if args.dataset_file == 'ytvos':
    ...
    model = ReferFormer(
        ...
        has_class_category_embed=args.has_class_category_embed,
    ...
    if args.masks: # always true
        ...
        weight_dict['loss_active_passive'] = args.cls_active_passive_loss_coef
    ############## sce/gce    
    # parser.add_argument('--cls_gce_loss_coef', default=2, type=float)
    # parser.add_argument('--cls_sce_loss_coef', default=2, type=float)
    ####use_gce#use_gce_weights#use_sce#use_sce_weights
        weight_dict['loss_sce'] = args.cls_sce_loss_coef
        weight_dict['loss_gce'] = args.cls_gce_loss_coef   
    ...
    if args.has_class_category_embed:
        weight_dict['loss_category_labels'] = args.cls_category_loss_coef
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
    ...
    if args.has_class_category_embed:
        losses = ['category_labels','labels', 'boxes']#
    else:
        losses = ['labels', 'boxes']
    if args.use_positive_cls:
        losses += ['positive_labels']
    if args.masks:
        if args.use_weights or args.use_sce_weights or args.use_gce_weights or args.use_sce or args.use_gce or args.use_active_passive:
            if args.use_weights:#use_sce_weights#use_gce_weights#use_weights#use_sce#use_gce
                losses += ['weighted_masks']##使用focal loss with weights
            if args.use_sce_weights:
                losses += ['weighted_sce_masks']
            if args.use_gce_weights:
                losses += ['weighted_gce_masks']
            if args.use_sce:
                losses += ['sce_masks']
            if args.use_gce:
                losses += ['gce_masks']
            if args.use_active_passive:
                losses += ['active_passive_masks']
        else:
            losses += ['masks']   ###使用rvos的 mask loss         
#### losses列表从下面的键中选择
    ...
    if args.use_elr_loss_mask:
        num_examp = 18000#args.num_frames * args.num_queries;
        cls_target_dir = os.path.join(args.output_dir, "cls_target")
        seg_target_dir = os.path.join(args.output_dir, "seg_target")
        ######### SetCriterionELROrgSizeMask
        criterion = SetCriterionELROrgSizeMask(
                num_examp,
                num_classes, 
                num_frames=args.num_frames,
                matcher=matcher,
                ...
                cls_target_dir=cls_target_dir,
                seg_target_dir=seg_target_dir,)
        criterion.to(device)
    elif args.use_npn_loss_mask:
        num_examp = 18000#args.num_frames * args.num_queries;
        candidate_dir_pos = os.path.join(args.output_dir, "candidate_cls_pos")
        candidate_dir_masks = os.path.join(args.output_dir, "candidate_masks")
        criterion = SetCriterionNPNOrgSizeMask(
            num_examp, 
            num_classes, 
            num_frames=args.num_frames,
            matcher=matcher, 
            ...
            losses=losses,
            focal_alpha=args.focal_alpha,
            warmup_epochs=args.warmup_epochs_npn, start_epoch=args.start_epoch,
            device=device, candidate_dir_pos=candidate_dir_pos, 
            candidate_dir_masks=candidate_dir_masks,
            )
        criterion.to(device)    
    else:

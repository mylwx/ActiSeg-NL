...
class ActionVOSDataset(Dataset):
    ...
    def __init__(self, actionvos_folder: Path, imagesets_path: Path, jpegimages_path: Path, 
                 ...
                 return_masks: bool, num_frames: int, max_skip: int, use_weights: bool, image_set: str ,clean_annotations_path: Path = None):   # ← Newly added
        ...
        print('\n')
        self.clean_annotations_path = clean_annotations_path  # ← Newly added
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            for j in range(self.num_frames):
                ...
                ##############
                # ↓ For visualization: only record paths
                debug_image_paths.append(img_path)
                if self.clean_annotations_path is not None:
                    clean_mask_path = os.path.join(str(self.clean_annotations_path), self.image_set, video, frame_name + '.png')
                    debug_clean_mask_paths.append(clean_mask_path)
                else:
                    debug_clean_mask_paths.append(None)
                ##############    
                if self.use_weights:
            ...
            target = {
                ...
                ### Visualization
                'video': video,
                'frame_names': torch.tensor(sample_indx),  # Or keep as Python list
                # For visualization only#
                'debug_image_paths': debug_image_paths,
                'debug_clean_mask_paths': debug_clean_mask_paths,
                ###########################
            }
...
def build(image_set, args):
    ...
    # Verify if paths exist (optional, add as needed)
    ...
    ##############################
    ###Visualization
    clean_annotations_path = None
    if hasattr(args, 'clean_annotations_path') and args.clean_annotations_path is not None:
        clean_annotations_path = Path(args.clean_annotations_path)
        assert clean_annotations_path.exists(), f'Clean Annotations path {clean_annotations_path} does not exist'
    ##############################
    ...
    dataset = ActionVOSDataset(
        ...
        image_set=image_set,
        ##############################
        clean_annotations_path=clean_annotations_path,  # ← Newly added
        ##############################
    )

...
# If random is not imported at file header, add one line:
import random
...
# Modify function signature to get args
...
#             break
#         idx = idx + 1
        # Add step mode logic at beginning of each step in training loop
        ##############################################
        # Dynamic freezing: only enabled when step mode is selected; DDP/non-DDP adaptive
        if getattr(args, 'use_pmhm', 1) and getattr(args, 'n_aux_heads', 1) > 0 and args.aux_freeze_mode == 'step':
            n_aux = int(getattr(args, 'n_aux_heads', 1))
            flags = [(random.random() < args.aux_freeze_step_p) for _ in range(n_aux)]
            m = model.module if hasattr(model, 'module') else model
            if hasattr(m, 'set_aux_freeze_flags'):
                m.set_aux_freeze_flags(flags)

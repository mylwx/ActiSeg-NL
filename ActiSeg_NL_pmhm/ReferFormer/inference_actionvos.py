...
# build transform
transform = T.Compose([
    T.Resize(360),#Change shortest side to 360, keep aspect ratio unchanged
    T.ToTensor(),
...
def main(args):
    ...
    img_folder = os.path.join(root, "JPEGImages_Sparse", split)#visor
    if args.actionvos_path[-1].split("_")[-1] in ['vost','vscos']:
        img_folder = os.path.join(root, "JPEGImages", split)#vost/vscos
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for video in video_list:
        ...
        for i in range(num_expressions):
            ...
            for clip_id in range(0, video_len, 36):
                ...
                for t in clip_frames_ids:
                    frame = frames[t]
                    # /root/project/1030/ActionVOS_ed/ActionVOS/dataset_vost/JPEGImages/val/00000001_10625_knead_dough_knead_dough/frame00000.jpg
                    print("img_folder = {}, video_name = {}, frame = {}".format(img_folder, video_name, frame))
                    img_path = os.path.join(img_folder, video_name, frame + ".jpg")
                    img = Image.open(img_path).convert('RGB')
                    origin_w, origin_h = img.size#Used for interpolating output results to get predicted annotations
                ...
                with torch.no_grad():
                    outputs = model([imgs], [exp], [target])#####Is it necessary to input multiple frames
                
                pred_logits = outputs["pred_logits"][0] ##class id
                pred_boxes = outputs["pred_boxes"][0]   # box 
                pred_masks = outputs["pred_masks"][0]   # mask
                ...
                if args.use_positive_cls:##cls positive label
                    if outputs['pred_positives'][0].sigmoid() < args.pos_cls_thres:#########classfication head label, for Acc
                        pred_masks.zero_()
        ...
        for f in range(out_masks.shape[0]):
            ...
            img_E.putpalette(palette)#modification here to remove palette

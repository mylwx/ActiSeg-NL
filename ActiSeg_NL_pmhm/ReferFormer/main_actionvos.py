...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        #####Update annealing progress of robust loss at beginning of each epoch in training loop
        ########################################################
        # 1) Annealing progress
        if hasattr(criterion, 'update_epoch'):
            criterion.update_epoch(epoch, args.epochs)

        # 2) Dynamic freezing, only enabled in epoch mode
        if getattr(args, 'use_pmhm', 1) and getattr(args, 'n_aux_heads', 1) > 0 and args.aux_freeze_mode == 'epoch':
            p0, p1 = args.aux_freeze_p0, args.aux_freeze_p1
            ratio = epoch / max(1, args.epochs - 1)
            p_freeze = p0 + (p1 - p0) * ratio
            n_aux = int(getattr(args, 'n_aux_heads', 1))
            flags = [(random.random() < p_freeze) for _ in range(n_aux)]
            if hasattr(model_without_ddp, 'set_aux_freeze_flags'):
                model_without_ddp.set_aux_freeze_flags(flags)

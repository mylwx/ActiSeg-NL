...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
                    ...
                    # ===== PMHM New Parameters =====
                    use_pmhm=True,
                    n_aux_heads=1,                  # 0 or 1 or 2
                    init_mode_single="copy_perturb",# "random" or "copy_perturb"
                    copy_sigma=1e-3,
                    aux_p_drop=0.2,                 # Aux heads only
                    aux_gamma_noise=0.2,            # Aux heads only
                    use_proto=False,                 # Optional prototype consistency uses mask_features
                    pmhm_viz_images=False
                    ):
        ...
        # self.tokenizer = RobertaTokenizerFast.from_pretrained(model_dir, local_files_only = True)
        # self.text_encoder = RobertaModel.from_pretrained(model_dir, local_files_only = True)
        ...
        ##### After "Build Dynamic Conv", insert PMHM members after controller definition
        ###########################
        # ===== PMHM Components =====
        self.use_pmhm = bool(use_pmhm)
        self.n_aux_heads = int(max(0, n_aux_heads))
        self.init_mode_single = str(init_mode_single)
        self.copy_sigma = float(copy_sigma)
        self.aux_p_drop = float(aux_p_drop)
        self.aux_gamma_noise = float(aux_gamma_noise)
        self.use_proto = bool(use_proto)

        # Aux head controllers, same structure as main head
        self.controller_aux = nn.ModuleList()
        for _ in range(self.n_aux_heads):
            aux = MLP(hidden_dim, hidden_dim, self.num_gen_params, 3)
            for layer in aux.layers:
                nn.init.zeros_(layer.bias)
                nn.init.xavier_uniform_(layer.weight)
            self.controller_aux.append(aux)

        # Feature perturbation module for aux heads only
        self.aux_dropout2d = nn.Dropout2d(self.aux_p_drop)

        # Initialization strategy
        self._init_aux_heads_from_main()

        # Dynamic freezing flags for aux heads only
        self._aux_freeze_flags: List[bool] = [False for _ in range(self.n_aux_heads)]

        ############################

        ############################
        #### Visualization
        self.pmhm_viz_images = bool(pmhm_viz_images)
        ############################

    ## Add three small methods inside ReferFormer class
    #####################################
    # ===== Initialization Strategy =====
    def _init_aux_heads_from_main(self):
        if self.n_aux_heads == 0:
            return
        with torch.no_grad():
            if self.n_aux_heads == 1:
                if self.init_mode_single == "copy_perturb":
                    self._copy_and_perturb(self.controller, self.controller_aux[0], self.copy_sigma)
                else:
                    pass  # Keep random initialization
            elif self.n_aux_heads == 2:
                # aux0 random; aux1 copy from main head with perturbation
                self._copy_and_perturb(self.controller, self.controller_aux[1], self.copy_sigma)

    @staticmethod
    def _copy_and_perturb(src: nn.Module, dst: nn.Module, sigma: float):
        for p_src, p_dst in zip(src.parameters(), dst.parameters()):
            p_dst.copy_(p_src)
            if sigma > 0:
                p_dst.add_(torch.randn_like(p_dst) * sigma)

    # ===== Dynamic freezing interface for aux heads only =====
    def set_aux_freeze_flags(self, flags: List[bool]):
        assert len(flags) == self.n_aux_heads
        self._aux_freeze_flags = list(flags)
        for flag, aux in zip(flags, self.controller_aux):
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if self.training and self.pmhm_viz_images:
            imgs_btchw = samples.tensors
        #################################
        # Export viz_rgb at top of forward() after samples becomes NestedTensor
        if self.training and self.pmhm_viz_images:
            mean = torch.tensor([0.485, 0.456, 0.406], device=imgs_btchw.device).view(1,1,3,1,1)
            std  = torch.tensor([0.229, 0.224, 0.225], device=imgs_btchw.device).view(1,1,3,1,1)
            # Rearrange by (b,t) and denormalize
            viz_rgb = rearrange(imgs_btchw, '(b t) c h w -> b t c h w', b=b, t=t)
            viz_rgb = (viz_rgb * std + mean).clamp(0,1)
            # Move to CPU/uint8 to avoid GPU memory usage
            viz_rgb = (viz_rgb * 255).byte().cpu()
            # Store in out for criterion visualization hook
            out = {} if 'out' not in locals() else out
            out['viz_rgb'] = viz_rgb
        ...
        ## Used for parallel aux heads during training, only last layer features, apply feature perturbation to aux heads
        ##############################################################
        # ===== Parallel aux heads: enabled during training, last layer only, single forward without clean-noise two paths =====
        if self.training and self.use_pmhm and self.n_aux_heads > 0:
            lvl = hs.shape[0] - 1
            lvl_references = inter_references[lvl, ..., :2]
            lvl_references = rearrange(lvl_references, '(b t) q n -> b (t q) n', b=b, t=t)
            aux_masks_list = []
            for aux in self.controller_aux:
                dyn_params_aux = aux(hs[lvl])                       # [(B*T), Q, num_params]
                dyn_params_aux = rearrange(dyn_params_aux, '(b t) q n -> b (t q) n', b=b, t=t)

                # Intra-head feature perturbation: aux heads only
                feat = mask_features
                feat = self.aux_dropout2d(feat)
                if self.aux_gamma_noise > 0 and self.training:
                    noise = torch.randn_like(feat) * self.aux_gamma_noise * (feat.detach().var(dim=(2,3,4), keepdim=True).sqrt() + 1e-6)
                    feat = feat + noise

                aux_mask = self.dynamic_mask_with_coords(feat, dyn_params_aux, lvl_references, targets)
                aux_mask = rearrange(aux_mask, 'b (t q) h w -> b t q h w', t=t)
                aux_masks_list.append(aux_mask)
            out['pred_masks_heads'] = aux_masks_list  # list, length = n_aux_heads
        ...
        ## At end of forward, if use_proto enabled, output mask_features
        ##############################################################
        # Optional: prototype consistency needs mask_features
        if self.training and self.use_proto:
            out['mask_features'] = mask_features  # [B, T, C, H', W']
...
def build(args):
    ...
    model = ReferFormer(
        ...
        # ===== Pass PMHM parameters =====
        use_pmhm=getattr(args, "use_pmhm", True),
        n_aux_heads=getattr(args, "n_aux_heads", 1),
        init_mode_single=getattr(args, "init_mode_single", "copy_perturb"),
        copy_sigma=getattr(args, "copy_sigma", 1e-3),
        aux_p_drop=getattr(args, "aux_p_drop", 0.2),
        aux_gamma_noise=getattr(args, "aux_gamma_noise", 0.2),
        use_proto=getattr(args, "use_proto", False),
        pmhm_viz_images=getattr(args, "pmhm_viz_images", False),  # ← Newly added
    ...
    ## After weight_dict construction, append PMHM loss coefficients (only when PMHM enabled)
    ####################################################
    # ===== NEW: PMHM Loss Weights =====
    if getattr(args, "use_pmhm", True):
        weight_dict['loss_tv'] = getattr(args, "tv_loss_coef", 0.3)
        weight_dict['loss_head'] = getattr(args, "head_loss_coef", 0.1)
        weight_dict['loss_layer'] = getattr(args, "layer_loss_coef", 0.1)
        if getattr(args, "use_proto", False):
            weight_dict['loss_proto'] = getattr(args, "proto_loss_coef", 0.05)        
    ####################################################
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
    ...
    #     else:
    #         losses += ['masks']
    ## After losses = ['labels', 'boxes'], at mask branch, switch original masks to robust version
    ####################################################
    if args.masks:
        if getattr(args, "use_pmhm", True):
            losses += ['masks_robust']   # Replace traditional masks
        else:
            if args.use_weights:
                losses += ['weighted_masks']
            else:
    ...
    ## At the end of criterion = SetCriterion(... parameters, pass PMHM hyperparameters
    criterion = SetCriterion(
            ...
            # ===== Pass PMHM hyperparameters =====
            use_pmhm=getattr(args, "use_pmhm", True),
            k_agree=getattr(args, "k_agree", 2),
            tau_m_p=getattr(args, "tau_m_p", 0.20),
            tau_e_p=getattr(args, "tau_e_p", 0.85),
            tau_h_p=getattr(args, "tau_h_p", 0.60),
            tv_alpha_start=getattr(args, "tv_alpha_start", 0.7),
            tv_alpha_end=getattr(args, "tv_alpha_end", 0.5),
            tv_beta=getattr(args, "tv_beta", 0.5),
            use_proto=getattr(args, "use_proto", False),
            # ---- These three newly added: statistics and visualization ----
            pmhm_log_stats=getattr(args, "pmhm_log_stats", False),
            pmhm_viz_images=getattr(args, "pmhm_viz_images", False),
            pmhm_viz_every=getattr(args, "pmhm_viz_every", 2000),
            pmhm_viz_max=getattr(args, "pmhm_viz_max", 20),
            pmhm_viz_dir=os.path.join(args.output_dir, getattr(args, "pmhm_viz_dir", "pmhm_viz")),
            )
    criterion.to(device)

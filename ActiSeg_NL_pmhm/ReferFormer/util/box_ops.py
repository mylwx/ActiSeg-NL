...
def generalized_box_iou(boxes1, boxes2):
    ...
    try:
        # Try to check conditions before executing assertion statements
        assert (boxes1[:, 2:] >= boxes1[:, :2]).all(), "Invalid boxes1 coordinates"
        ...
        print("Boxes2 info:")
        print(boxes2)
        raise  # Re-raise exception for subsequent processing
    # assert (boxes1[:, 2:] >= boxes1[:, :2]).all()
    # assert (boxes2[:, 2:] >= boxes2[:, :2]).all()
    # iou, union = box_iou(boxes1, boxes2)

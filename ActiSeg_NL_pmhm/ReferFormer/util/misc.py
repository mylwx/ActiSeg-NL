...
def collate_fn_ws(batch):####Used in main_actionvos.py
    # batch: imgs_w,imgs_s, targets
    batch = list(zip(*batch)) 
    batch[0] = nested_tensor_from_videos_list(batch[0], size_divisibility=32) 
    batch[1] = nested_tensor_from_videos_list(batch[1], size_divisibility=32) 
    # batch[0]: samples: NestedTensor(tensor, mask)###weak augmentation
    #           tensor: [B, T, C, H, W], mask: [B, T, H, W]
    # batch[1]: samples: NestedTensor(tensor, mask)###strong augmentation
    #           tensor: [B, T, C, H, W], mask: [B, T, H, W]
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    excluded_keys = [
        ...
        "task_id",
        "original_id",
        # ↓ Your newly added visualization fields (pure Python objects, don't touch)
        "video", "debug_image_paths", "debug_clean_mask_paths",
    ]
    """
    if "dataset_name" in targets[0]:
        # for ["refcoco", "refcoco+", "refcocog"] evaluation
        return [{k: v.to(device)  for k, v in t.items() if k not in ["caption", "dataset_name", "original_id","video", "debug_image_paths", "debug_clean_mask_paths",]} for t in targets]
    else:
        return [{k: v.to(device)  for k, v in t.items() if k not in ["caption", "dataset_name", "original_id", "image_id","video_id","video", "debug_image_paths", "debug_clean_mask_paths",]} for t in targets]
    # if "dataset_name" in targets[0]:
    #     # for ["refcoco", "refcoco+", "refcocog"] evaluation

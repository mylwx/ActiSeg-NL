...
def cal_gres_all(gt_path,pred_path,split_json,filter=True):
    ...
    # print('pos-mIoU:  {:.6f}'.format(sum(mious_pos)/len(mious_pos)))
    # print('neg-mIoU:  {:.6f}'.format( sum(mious_neg)/len(mious_neg)))
    # print('pos-cIoU:  {:.6f}'.format( sum(cis_pos)/sum(cus_pos)))
    # print('neg-cIoU:  {:.6f}'.format( sum(cis_neg)/sum(cus_neg)))
    # print('gIoU:      {:.6f}'.format( sum(gious)/len(gious)))
    # print('acc:       {:.6f}'.format( (TN+TP)/(TN+TP+FN+FP)))
    # print('TN,TP,FN,FP: {},{},{},{}'.format( TN,TP,FN,FP))
    # print(f'------------------------------------')
    print('{:.6f},'.format(sum(mious_pos)/len(mious_pos)))
    print('{:.6f},'.format( sum(mious_neg)/len(mious_neg)))
    print('{:.6f},'.format( sum(cis_pos)/sum(cus_pos)))
    print('{:.6f},'.format( sum(cis_neg)/sum(cus_neg)))
    print('{:.6f},'.format( sum(gious)/len(gious)))
    print('{:.6f},'.format( (TN+TP)/(TN+TP+FN+FP)))
